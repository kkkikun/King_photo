...
## 文档管理和版本控制规则（v1.7.1新增）

### 15.1 文档分类管理

**规则：** 项目文档分为两类，采用不同的版本控制策略：

| 文档类型 | 示例文件 | GitHub管理 | 说明 |
|----------|----------|------------|------|
| **正式文档** | README.md, PROJECT_STRUCTURE.md, DEVELOPMENT_RULES.md, PLUGIN_DOC.md, API.md, error_solutions.md | ✅ 必须上传 | 项目核心文档，需要版本控制 |
| **实现方案** | *_实现方案.md, *_设计方案.md, *_分析报告.md | ❌ 不上传 | 开发过程文档，添加到.gitignore |

### 15.2 实现方案文档管理

**规则：** 实现方案类文档不需要上传到GitHub。
...
1. 在项目根目录的 `.gitignore` 文件中添加规则：
   ...
   *_计划书.md
   ```
2. 这些文档保留在本地，用于开发参考
3. 包含敏感信息的文档必须添加到.gitignore

### 15.3 README.md 更新规则

**规则：** 每次进行大更新后，必须同步更新README.md文件。

**大更新定义：**
- 版本号变更（如v1.7.0 → v1.7.1）
- 新增重要功能或模块
- 项目架构重大调整
- 依赖项重大变更
- 文件结构重大变化
...
**更新内容：**
1. **版本历史**：在"更新日志"部分添加新版本记录
2. **功能特性**：更新功能列表，添加新功能描述
3. **文件结构**：更新目录结构图，添加新文件
4. **安装说明**：更新依赖项和安装步骤（如有变化）
5. **使用说明**：更新使用示例（如有变化）

**更新流程：**
```bash
# 1. 检查是否需要更新README
grep -n "v1.7.0" README.md  # 检查旧版本号
...
# 5. 提交更改
git add README.md
git commit -m "docs: 更新README.md至v1.7.1"
```
...
| 版本类型 | 格式 | 使用场景 |
|----------|------|----------|
| 主版本号 | X.0.0 | 重大架构变更、不兼容的API修改 |
| 次版本号 | 0.X.0 | 新增功能、向下兼容的功能性更新 |
| 修订号 | 0.0.X | Bug修复、小改动、文档更新 |

**版本更新检查清单：**
- [ ] 更新 `README.md` 中的版本号
- [ ] 更新 `PROJECT_STRUCTURE.md` 中的版本号
- [ ] 更新 `DEVELOPMENT_RULES.md` 中的版本号
- [ ] 更新 `PLUGIN_DOC.md` 中的版本号
- [ ] 更新 `API.md` 中的版本号
- [ ] 更新 `error_solutions.md` 中的版本号
- [ ] 检查并更新所有 `*_实现方案.md` 文件（如有引用版本号）
- [ ] 检查 `.gitignore` 文件是否需要更新

### 15.5 Git提交规范

**规则：** 提交信息遵循 Conventional Commits 规范：
...
**示例：**
```
feat(api): 添加批量元数据写入API

- 新增 batch_write_metadata() 方法
- 支持进度回调和取消功能
- 更新API文档
